...
# Script de gestión para el módulo de visualización de Pig
# Uso: ./pig-visualization.sh [comando]
...
index_pig_data() {
    ...
    if command -v node &> /dev/null; then
        ...
        echo -e "${YELLOW}⚠️  Node.js no disponible, usando Docker...${NC}"
...
start_server() {
    echo -e "${BLUE}🖥️  Iniciando servidor de visualización...${NC}"
...
start_docker() {
    ...
    # Indexar datos si están disponibles
    if check_pig_data; then
        index_pig_data
    fi
    
    # Levantar servicio de visualización
...
check_status() {
    ...
    if curl -s -f http://localhost:9200/_cluster/health > /dev/null; then
        ...
        if curl -s -f "http://localhost:9200/waze-pig-metrics" > /dev/null; then
            ...
            echo -e "${YELLOW}⚠️  Índice waze-pig-metrics: No existe${NC}"
        fi
    else
...
clean_elasticsearch() {
    ...
    for index in "${indices[@]}"; do
        if curl -s -f "http://localhost:9200/$index" > /dev/null; then
            echo -e "${YELLOW}🗑️  Eliminando índice $index...${NC}"
            curl -X DELETE "http://localhost:9200/$index"
            echo -e "${GREEN}✅ Índice $index eliminado${NC}"
        else
            echo -e "${BLUE}ℹ️  Índice $index no existe${NC}"
        fi
    done
}
